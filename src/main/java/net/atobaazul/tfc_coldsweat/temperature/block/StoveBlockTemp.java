...
public class StoveBlockTemp extends BlockTemp {
    public StoveBlockTemp() {
        super(TFCBlocks.STOVE.get());
    }
    ...
    public double getTemperature(Level level, LivingEntity entity, BlockState state, BlockPos pos, double distance) {
        ...
        if (be instanceof FirepitBlockEntity) {
            return CSMath.blend(Temperature.convert(((FirepitBlockEntity) be).getTemperature(), Temperature.Units.C, Temperature.Units.MC, true), 0, distance, 0.5, 16) / 30;
        }
        return 0.0;
    }
